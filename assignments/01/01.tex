...
\\\\
\textbf{Maintainance} \mdseries Each consecutive iteration places $A[j]$ at
the $A[j-1]$ whenever $A[j] < A[j-1]$ maintaining the loop invariant
property given in the initialization phase, that
$A[j-1] = min{A[j-1 \dots n]}$ at the end of each iteration.
\\\\
...
...
\\\\
\textbf{Initialization} \mdseries ...
\\\\
\textbf{Maintainance} \mdseries ...
\\\\
